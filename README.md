...
- Vertex AI mode does not require `ANTHROPIC_API_KEY`; set `KB_LLM_PROVIDER=vertex` or `CLAUDE_CODE_USE_VERTEX=1`, configure `ANTHROPIC_VERTEX_PROJECT_ID` and `CLOUD_ML_REGION`, then authenticate with Google Application Default Credentials or `GOOGLE_APPLICATION_CREDENTIALS`.
...
Env: `ANTHROPIC_API_KEY`, `KB_LLM_PROVIDER`, `CLAUDE_CODE_USE_VERTEX`, `ANTHROPIC_VERTEX_PROJECT_ID`, `CLOUD_ML_REGION`, `GOOGLE_APPLICATION_CREDENTIALS`, `KB_DIR`, `KB_WORKSPACES`, `KB_MODEL`, `KB_TOKEN_BUDGET`, `KB_PERMISSION_MODE`, `KB_NO_COMMIT`, `KB_COLOR`.
